        ...
        def organize_data(self,data_CE,data_nCE,fMT,metal,chunk):
                ...
                for i in fMT:
                        ...
                        for j in metal:
                         ...
                         # Concatenate the last three metallicities
                        df_metal_u_CE = pd.concat(df_metal_CE[-3:],ignore_index=True)
                        df_metal_CE = df_metal_CE[:-3]
                        df_metal_CE.append(df_metal_u_CE)
                        df_metal_u_nCE = pd.concat(df_metal_nCE[-3:],ignore_index=True)
                        df_metal_nCE = df_metal_nCE[:-3]
